    ...
    return jsonify({
        ...
        "endpoints": {
            "/api/token": {
                "method": "GET",
                "params": {
                    "uid": "string (required with password)",
                    "password": "string (required with uid)",
                    "access_token": "string (optional, alternative to uid+password)"
                }
            }
        }
    })
...
# The app object is ready; Vercel expects the variable 'app'.
# For Vercel serverless, a handler is provided.
